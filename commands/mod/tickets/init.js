...
class Init extends Command {
  constructor() {
    ...
    this.categoriesEmbed = EmbedGenerator.createSmallEmbed(
      ":pencil:",
      `How many categories u want to add? (min:1, max:20)`,
      BLUE
    );
    this.categoryNameEmbed = EmbedGenerator.createSmallEmbed(
      ":pencil:",
      `Please type category name (max characters:60, type exit to terminate this)`,
      BLUE
    );
    ...
    this.embedTextEmbed = EmbedGenerator.createSmallEmbed(
      ":pencil:",
      `Type embed text (max length 600 characters)`,
      BLUE
    );
    ...
    this.rememberInfo = EmbedGenerator.createSmallEmbed(
      ":wrench: ",
      `You must remember about discord limits, max 50 channels per category.\n | So u must delete tickets when closed before limit is reached. \n   | Archived tickets is comming.`,
      RED
    );
  }
  async run(client, message, args) {
    ...
    do {
      tryAgain = false;
      mtd.push(await message.channel.send(this.categoriesEmbed));
